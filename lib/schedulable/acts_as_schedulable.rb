          ...
          define_method "build_#{occurrences_association}" do
            ...
            if schedule.present?
              ...
              occurrences_records = schedulable.send("remaining_#{occurrences_association}")
							total_records = occurrences_records.length
              # build occurrences
              existing_record = nil
              occurrences.each_with_index do |occurrence, index|
                ...
                begin
                  if existing_records.any?
                    ...
                    # Create new record
                    record = occurrences_records.create(date: occurrence.to_datetime)
                    if !record.persisted?
                      puts 'An error occurred while creating an occurrence record'
                    else
                      puts "Created new record #{record.id}"
                    end
                  end
              ...
              occurrences_records = schedulable.send("remaining_#{occurrences_association}").limit(1000)
              record_count = 0
              total_records = occurrences_records.length
              occurrences_records.find_each.with_index do |occurrence_record, index|
